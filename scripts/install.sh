...
docker build -t confluentinc/cp-server-vf:7.5.2 "${DOCKER_IMAGE_DIR}/kafka"
# Build custom Kafka Connect image
docker build -t confluentinc/cp-server-connect-vf:7.5.2 "${DOCKER_IMAGE_DIR}/connect"
# Build custom Schema Registry image
docker build -t confluentinc/cp-schema-registry-vf:7.5.2 "${DOCKER_IMAGE_DIR}/schema-registry"
...
helm upgrade --install mysql bitnami/mysql --version 9.14.1 -f "${TUTORIAL_HOME}/manifests/mysql-values.yaml"
...
helm upgrade --install mariadb bitnami/mariadb --version 14.1.0 -f "${TUTORIAL_HOME}/manifests/mariadb-values.yaml"
