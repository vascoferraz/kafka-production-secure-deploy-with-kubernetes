...
docker build -t confluentinc/cp-server-vf:7.4.0 $TUTORIAL_HOME/docker-images/kafka

# Build custom Kafka Connect image
docker build -t confluentinc/cp-server-connect-vf:7.4.0 $TUTORIAL_HOME/docker-images/connect
...
helm upgrade --install postgresql bitnami/postgresql --version 12.5.7 -f $TUTORIAL_HOME/manifests/postgres-values.yaml
...
helm upgrade --install mysql bitnami/mysql --version 9.10.1 -f $TUTORIAL_HOME/manifests/mysql-values.yaml
